...
@RunWith(JMock.class)
public class AuctionSniperLauncherTest {
  private final Mockery context = new Mockery();
  private final States auctionState = context.states("auction state").startsAs("not joined");
  private final Auction auction = context.mock(Auction.class);
  private final AuctionHouse auctionHouse = context.mock(AuctionHouse.class);
  private final AuctionSniperCollector auctionSniperCollector = context.mock(AuctionSniperCollector.class);
  private final AuctionSniperLauncher launcher = new AuctionSniperLauncher(auctionHouse, auctionSniperCollector);
  ...
  addsNewSniperToCollectorAndThenJoinsAuction() {
    ...
    context.checking(new Expectations() {{
      ...
      oneOf(auctionSniperCollector).addSniper(with(sniperForItem(auctionItem))); when(auctionState.is("not joined"));
